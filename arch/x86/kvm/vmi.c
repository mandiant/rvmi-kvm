...
void kvm_vmi_vcpu_uninit(struct kvm_vcpu *vcpu)
{
	...
	struct vmi_gfn_entry *gfn_entry;
	...
	hash_for_each_safe (vcpu->vmi_gfn_ht, i, tmp, gfn_entry, list) {
		hash_del(&gfn_entry->list);
		kfree(gfn_entry);
	}
}
